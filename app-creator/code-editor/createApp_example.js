...
/**
 * AppCreator.createApp provides an API for building a single app instance from the app creator (https://ee-app-creator.appspot.com/).
 * This method takes in a template string exported from the website and renders the corresponding earth engine widgets on the code editor.
 * As an example, a label widget from the app creator will be converted into a ui.Label() widget on the code editor. A widget interface
 * is provided by the API to access the corresponding UI elements.
 * -----------------------------------------------------------------------------------------------
 *
 * In the following example we're going to look at how to build an app instance and access the corresponding UI elements.
 */

/**
 * 1. Create app instance by pasting the template string provided by the app creator.
 */
...
/**
 * 2. At this point, we can directly access the widgets by calling app.widgets(). This will return an ActiveDictionary
 * of all the corresponding widgets. Using this interface, we can bind actions to any of the underlying elements.
 * Lets bind an action to the button with id 'button-0'.
 */
app
  .widgets()
  .get('button-0')
...
/**
 * To view the different widgets available, Lets print the widgets interface to the console like so.
 */
print(app.widgets());
