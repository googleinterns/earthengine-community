...
/**
 * 2. Binding actions to our app instances. We can view the list of widgets available by printing the widgets interface
 * for each app like so.
 *
 * print(desktop.widgets())
 * print(mobile.widgets())
 *
 * We can then bind our actions by specifying a particular widget by its id. In the following example, we are going to look at
 * how we can bind an onClick handler on each of our buttons.
 */
